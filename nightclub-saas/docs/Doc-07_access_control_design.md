...
### 3-5. SystemAdmin（SaaS運営者）

- 全テナント横断で読み取り可能（監査・障害対応目的）
- 全テナント横断で限定的に書き込み可能（テナント作成、停止、移行、監査修正）
- 売上や給与の通常運用入力はしない（原則禁止）
- 締め後修正は ChangeRequest 承認フローを踏む。ただし緊急時の例外手順を許容し、その場合は監査ログを必須とする
...
- JWT内 `tenantId` とリクエスト対象 `tenantId` が一致しない場合 → **404 または 403** (Doc-24方針に合わせる)
- **例外**: `tenantId` 不一致を許可できるのは SystemAdmin のみとする。SystemAdminが跨ぎアクセスする場合は必ず AuditLog に `reason` を残す（フィールド追加はPhase2でもよいが方針として明記する）。
